...
static const char *client_error_strings[RABBITMQ_C_ERROR_MAX] = {
...
{
  const char *str;
  int category = (err & RABBITMQ_C_ERROR_CATEGORY_MASK);
  err = (err & ~RABBITMQ_C_ERROR_CATEGORY_MASK);

  switch (category) {
  case RABBITMQ_C_ERROR_CATEGORY_CLIENT:
    if (err < 1 || err > RABBITMQ_C_ERROR_MAX)
      str = "(undefined librabbitmq error)";
    else
      str = client_error_strings[err - 1];
    break;

  case RABBITMQ_C_ERROR_CATEGORY_OS:
		       ...
		       amqp_bytes_t body)
{
...
#else
  {
	...
	nRC = amqp_send_method(state, channel, AMQP_BASIC_PUBLISH_METHOD, &m);
		   ...
		   amqp_boolean_t multiple)
{
...
#else
  {
    ...
    nRC = amqp_send_method(state, channel, AMQP_BASIC_ACK_METHOD, &m);
