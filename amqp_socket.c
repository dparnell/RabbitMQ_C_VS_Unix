			    ...
			    va_list vl)
{
...

  {
...
#else
    {
	  ...
	  nRC = amqp_send_method(state, 0, AMQP_CONNECTION_START_OK_METHOD, &s);
...
#else
  {
	...
	nRC = amqp_tune_connection(state, channel_max, frame_max, heartbeat);
	if( nRC < 0 )
	  return nRC;
  }
#endif

  {
...
#else
    {
	  ...
	  nRC = amqp_send_method(state, 0, AMQP_CONNECTION_TUNE_OK_METHOD, &s);
	  if( nRC < 0 )
	    return nRC;
	}
#endif
