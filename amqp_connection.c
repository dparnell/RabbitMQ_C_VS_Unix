 ...
 read_more:
  ...
  switch (state->state) {
...
#else
    {
	  ...
	  nRC = amqp_decode_method(decoded_frame->payload.method.id,
		                       &state->decoding_pool,
                               encoded,
                               &decoded_frame->payload.method.decoded);
...
#else
    {
	  ...
	  nRC = amqp_decode_properties(decoded_frame->payload.properties.class_id,
                                   &state->decoding_pool,
                                   encoded,
                                   &decoded_frame->payload.properties.decoded);
			    ...
			    int *payload_len)
{
  ...
  switch (frame->frame_type) {
...
#else
    {
	  ...
	  nRC = amqp_encode_method(frame->payload.method.id,
                               frame->payload.method.decoded,
                               *encoded);
      if( nRC < 0 )
        return nRC;
    ...
    case AMQP_FRAME_HEADER:
      ...
      *payload_len = AMQP_CHECK_RESULT(amqp_encode_properties(frame->payload.properties.class_id,
...
#else
      {
        ...
        nRC = amqp_encode_properties(frame->payload.properties.class_id,
                                     frame->payload.properties.decoded,
                                     *encoded);
        if( nRC < 0 )
          return nRC;
